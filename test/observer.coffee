...
class TestClient
  ...
  observeMultipleEventsByHash: ->
    @observe @emitter,
      'test:start
       test:step': @handleStart
      'test:raise': @handleRaise
      'test:end': @handleEnd
...
describe 'Observer', ->
  ...
  describe '#observe', ->
    ...
    it 'should be able to observe multiple events at a time in a hash', (done) ->
      client.observeMultipleEventsByHash()
      emitter.start()
      emitter.step()
      emitter.end()
      emitter.raise()
      if client.events.length is 3
        done()
      else
        done("Unexpected number of events.")
